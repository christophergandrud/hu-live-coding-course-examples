...
#+ message=FALSE
xfun::pkg_attach2("tidyverse", "dplyr")
theme_set(theme_minimal())

mobility <- read_csv("src/data/mobility-data.csv")
...
# Test for duplicates
mobility_distinct <- dplyr::distinct(de_mobility_long, 
                                     country_region_code,
                                     date,
                                     destination)

if (nrow(mobility_distinct) != nrow(de_mobility_long))
  stop("More rows than expected")



de_mobility_long %>%
  dplyr::group_by(country_region_code, date, destination) %>%
